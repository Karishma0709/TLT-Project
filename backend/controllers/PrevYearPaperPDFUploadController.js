...
async function createPyPaperPDFupload(req, res) {
  try {
    ...
    const paperimage = req.file.filename; // Multer automatically adds the file object

    await Pypschema.create({ Papertitle, paperimage });
    return res.status(200).json({ status: 'ok', message: 'Paper uploaded successfully' });
  } catch (error) {
...
const updatePyPaperPDFupload = async (req, res) => {
  try {
    ...
    const updatedPaper = await Pypschema.findByIdAndUpdate(id, updateData, { new: true });
